...
#else /* ENABLE_QMPI */
/*@
   MPI_Status_set_elements_c - Set the number of elements in a status

Input Parameters:
+ datatype - datatype associated with count (handle) (handle)
- count - number of elements to associate with status (integer) (integer)

Input/Output Parameters:
. status - status with which to associate count (Status) (Status)
