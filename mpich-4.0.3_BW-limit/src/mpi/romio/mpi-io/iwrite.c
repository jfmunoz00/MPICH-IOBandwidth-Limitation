                      ...
                      int count, MPI_Datatype datatype, char *myname, MPI_Request * request)
{
    ...
    if (buftype_is_contig && filetype_is_contig) {
        ...
        if (!(adio_fh->atomicity)) {
            // CHANGE
            //ADIO_IwriteContig(adio_fh, buf, count, datatype, file_ptr_type, off, request, &error_code);
            Async_IwriteContig(adio_fh, buf, count, datatype, file_ptr_type, off, request, &error_code);
            // END CHANGE
            ...
            // CHANGE
            // ADIO_WriteContig(adio_fh, buf, count, datatype, file_ptr_type, off, &status, &error_code);
            Async_WriteContig(adio_fh, buf, count, datatype, file_ptr_type, off, &status, &error_code);
            // END CHANGE
