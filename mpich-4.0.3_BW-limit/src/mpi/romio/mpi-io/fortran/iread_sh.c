                            ...
                            MPI_Datatype datatype, MPI_Request * request)
{
    ...
    if (buftype_is_contig && filetype_is_contig) {
        ...
        if (!(adio_fh->atomicity)) {
            // CHANGE
            // ADIO_IreadContig(adio_fh, buf, count, datatype, ADIO_EXPLICIT_OFFSET, off, request, &error_code);
            Async_IreadContig(adio_fh, buf, count, datatype, ADIO_EXPLICIT_OFFSET, off, request, &error_code);
            // END CHANGE
            ...
            // CHANGE
            // ADIO_ReadContig(adio_fh, buf, count, datatype, ADIO_EXPLICIT_OFFSET, off, &status, &error_code);
            Async_ReadContig(adio_fh, buf, count, datatype, ADIO_EXPLICIT_OFFSET, off, &status, &error_code);
            // END CHANGE
